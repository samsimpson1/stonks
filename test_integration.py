...
class TestScraperIntegration:
  ...
  def test_scraper_initialization(self, temp_db_path, scraper_process):
    ...
    if world_count == 0 and scraper_process.poll() is not None:
      ...
      pytest.fail(f"Scraper process failed. stderr: {stderr.decode()}, stdout: {stdout.decode()}")
  ...
  def test_database_schema(self, temp_db_path, scraper_process):
    ...
    expected_sales_columns = ["timestamp", "world_id", "item_id", "price", "quantity", "buyer"]
    ...
    assert any("timestamp" in idx for idx in indexes), "sales table should have timestamp index"
